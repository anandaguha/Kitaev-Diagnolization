...
class Relation_Table:
    ...
    def __init__ (self, neighbor_tabel:np.ndarray, total_bonds:int ,basis_bonds:tuple[int], basis_unit_cell:tuple[tuple[int]], reflection: bool = False ):
        ...
        total neighbors,
        the nearest neighbor bonds you want to use as a basis, 
        the unit cell basis vectors this needs to be given in terms of the nearest neighbor basis,
        if it is a symetric  for computation time a possible (not implemented yet)
        """
    ...
    def create_momentum_transform (self):
        ...
        def functional_momentum_matrix(k1,k2):
            for (row_idx, col_idx), value in np.ndenumerate(self._neighbor_table):
                ...
                bond_vector_neighbor_basis = np.ndarray([coefA],[coefB])
                neighbor_basis_to_unit_cell_basis = np.ndarray([unit_cell_bond_one_neighbor_basis[0],unit_cell_bond_one_neighbor_basis[1]],
                ...
                new_coef = bond_vector_neighbor_basis @ inverse_neighbor_to_unit_cell
